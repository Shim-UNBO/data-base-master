...
const SendingPage = ({memberList, setClose, checkedUsers}) => {
  ...
  const onFocusSelect =()=>{setSelectAlert('')}
  const onFocusTitle =()=>{setTitleAlert('')}
  const onFocusContent =()=>{setContentAlert('')}
    ...
    <Container >
      ...
      <Wrap>
        ...
        <Text >종류 선택 :</Text>
          ...
          <Text>이메일 제목 :</Text>
          <HeadWrap>
            <Input onChange={onChange(setTitle)} onFocus={onFocusTitle} type='text' placeholder='이메일 제목' />
            <Alert>{titleAlert}</Alert>
          </HeadWrap>
        <Text>이메일 내용 :</Text>
        <HeadWrap height='300px'>
          <TextArea onChange={onChange(setContent)} onFocus={onFocusContent} placeholder='이메일 내용'></TextArea>
