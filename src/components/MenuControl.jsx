...
const Management = () => {
  ...
  const handleSaveChanges = (event) => {
    ...
    axios
      .post('https://api.mever.me:8080/updateMenu', jsonData, {headers: {'Content-Type': 'application/json', },})
      .then((response) => {
  ...
  return (
    ...
    {menus.map((menu, index) => (
      <div className="menu" key={index}>
        ...
        <button className="menuDetails" onClick={() => handleViewDetails(menu.seq)}>
          수정하기
        </button>
      </div>
    ))}
    {selectedMenu && showPopup && (
      ...
      <button className="closeButton" onClick={handleSaveChanges}>
          수정완료
        </button>
